...
namespace NuBaultBank.Core.Services;
  ...
  public class ToDoItemSearchService : IToDoItemSearchService
  {
      ...
      public async Task<Result<List<ToDoItem>>> GetAllIncompleteItemsAsync(Guid projectId, string searchString)
      {
      ...
      public async Task<Result<ToDoItem>> GetNextIncompleteItemAsync(Guid projectId)
      {
          var projectSpec = new ProjectByIdWithItemsSpec(projectId);
          var project = await _repository.GetBySpecAsync(projectSpec);
          if (project == null)
